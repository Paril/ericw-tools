    ...
    const float &hotspot_clamp)
{
    ...
    for (const auto & surf_ptr : LightSurfaces()) {
        ...
        if (!surf_ptr->vpl) {
            ...
            // don't light ourself; postprocess will handle it
            continue;
        }
        ...
        for (const auto &vpl_setting : surf_ptr->vpl->styles) {
            ...
            if (SurfaceLight_SphereCull(&vpl, lightsurf, vpl_setting, surflight_gate, hotspot_clamp))
                continue;
    ...
    if (light_options.debugmode == debugmodes::none) {
        ...
        if (lightsurf.vpl) {
            if (auto value = IsSurfaceLitFace(bsp, face)) {
                ...
                auto &vpl = *lightsurf.vpl;

                auto *entity = std::get<3>(value.value());
                float surface_minlight_scale = entity ? entity->surflight_minlight_scale.value() : 1.f;
                surface_minlight_scale *= lightsurf.surflight_minlight_scale;

                const float standard_scale = cfg.surflightscale.value();
                const float sky_scale = cfg.surflightskyscale.value();

                // Get self-light contribution
                if (surface_minlight_scale > 0) {
                    int style = std::get<1>(value.value());

                    std::optional<qvec3f> result;

                    for (auto &vpl_settings : vpl.styles) {
                        if (vpl_settings.style != style) {
                            continue;
                        }

                        result = vpl_settings.color * vpl_settings.intensity * (vpl_settings.omnidirectional ? sky_scale : standard_scale) * (1.f / (16.f * 16.f));
                        break;
                    }
                    ...
                    if (result.has_value()) {
                        ...
                        for (auto &sample : lightmap->samples) {
                            hit = Light_ClampMin(sample, 255.f, result.value()) || hit;
                        }
