    ...
    for (i = 0; i < portalleafs_real; i++) {
        ...
        for (j = 0; j < portalleafs_real; j++) {
            ...
            for (k = 0; k < hit->nummarksurfaces; k++) {
                ...
                if (!Q_strncasecmp(miptex.name.data(), "sky", 3) && !settings::noambientsky.value())
                    ambient_type = AMBIENT_SKY;
                else if (!Q_strncasecmp(miptex.name.data(), "*water", 6) && !settings::noambientwater.value())
                    ambient_type = AMBIENT_WATER;
                else if (!Q_strncasecmp(miptex.name.data(), "*04water", 8) && !settings::noambientwater.value())
                    ambient_type = AMBIENT_WATER;
                else if (!Q_strncasecmp(miptex.name.data(), "*slime", 6) && !settings::noambientslime.value())
                    ambient_type = AMBIENT_WATER; // AMBIENT_SLIME;
                else if (!Q_strncasecmp(miptex.name.data(), "*lava", 5) && !settings::noambientlava.value())
